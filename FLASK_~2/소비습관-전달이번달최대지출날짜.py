...
month_ago = before_one_month.strftime("%Y-%m")

time = datetime.today().strftime("%Y-%m")
dataFilter = df['date'].str.contains(time)
result_thismonth = df[dataFilter]
dataFilter = df['date'].str.contains(month_ago)
result_lastmonth = df[dataFilter]

print(result_thismonth)
print(result_lastmonth)

u_lastmonth = result_lastmonth['name'] == uid
u_thismonth = result_thismonth['name'] == uid

print(result_lastmonth[u_lastmonth])
print(result_thismonth[u_thismonth])

lastmonth_list=[]
thismonth_list=[]
lastmonth_list.append(result_lastmonth[u_lastmonth]['date'].unique())
thismonth_list.append(result_thismonth[u_thismonth]['date'].unique())
...
##특정 날짜의 데이터프레임 출력방법
result = result_lastmonth[u_lastmonth]

for i in range(len(lastmonth_list)):
    print(result[result_lastmonth[u_lastmonth]['date'] == lastmonth_list[i]])



print(result[result_lastmonth[u_lastmonth]['date'] == '2022-08-10'])
#특정 날짜의 데이터프레임의 cost 출력방법
print(result[result_lastmonth[u_lastmonth]['date'] == '2022-08-10']['cost'].sum())
